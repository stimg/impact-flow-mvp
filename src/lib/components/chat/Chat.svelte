<script lang="ts">
    ...
    import {chatId, chats, chatTitle, config, currentChatPage, mobile, type Model, models, settings, showArtifacts, showCallOverlay, showControls, showOverview, showSidebar, socket, tags as allTags, temporaryChatEnabled, tools, toolServers, user, WEBUI_NAME} from '$lib/stores';
    import {convertMessagesToHistory, copyToClipboard, createMessagesList, getMessageContentParts, getPromptVariables, processDetails, promptTemplate, removeAllDetails} from '$lib/utils';
    ...
    import {chatAction, chatCompleted, generateMoACompletion, getTaskIdsByChatId, stopTask} from '$lib/apis';
    ...
    const processMicLevel = () => {
        ...
        const processFrame = () => {
            ...
            setLkMicLevel(Math.max(...domainData) / 256);
        };
    ...
    export const startLivekitAsr = async () => {
        ...
        try {
            ...
            console.log("[Chat] Audio input devices:", audioInputs);
            ...
            if (defaultDevice) {
                ...
                console.log("[Chat] Using audio input device:", defaultDevice.label);
            }
            ...
            lkMicStream = await navigator.mediaDevices.getUserMedia(constraints);
            const micTrack = lkMicStream.getAudioTracks()[0];
        ...
        try {
            if (lkRoom) {
                try {
                    ...
                    console.log('[Chat] Failed to disconnect room. Error:', e.message || e.toString())
                }
            }
            lkRoom = null;
