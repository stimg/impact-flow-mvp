<script lang="ts">
	...
	export const startLivekitAsr = async () => {
		...
		try {
			...
			room.on(RoomEvent.Connected, () => {
				...
				processMicLevel();
				console.log(
					`[FE] connected: room: ${room.name}, identity: ${room.localParticipant.identity}`
				);
	...
	const chatCompletionEventHandler = async (data, message, chatId) => {
		console.log('[Chat] chatCompletionEventHandler data:', data);
		const { id, done, choices, content, sources, selected_model_id, error, usage, audio } = data;
		...
		const res = await generateOpenAIChatCompletion(
			localStorage.token,
			{
				...
				features: {
					...
					audio_streaming: $showCallOverlay
				},
				variables: {
					...getPromptVariables(
						$user?.name,
...
>
	{#if !loading}
		<div in:fade={{ duration: 50 }} class="w-full h-full flex flex-col">
			...
			<div class="text-center text-sm text-gray-400 dark:text-gray-600 mb-7">
				&copy; {dayjs().year()} Impact Flow 🇨🇭 Version: 0.9.0 beta
			</div>
		</div>
	{:else if loading}
