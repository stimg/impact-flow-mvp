<script lang="ts">
    ...
    const getMetadata = (): Omit<Product, 'id'> => ({
    ...
    const onSubmit = async (evt: SubmitEvent) => {
        ...
        await generateEmbeddingsHandler(localStorage.token, id, getMetadata());
    ...
    const onResetForm = (evt: SubmitEvent) => {
        resetProductData(evt);
    }
...
<div>
    ...
    <form class="flex flex-col justify-between space-y-3 text-sm" on:submit|preventDefault={onSubmit} on:reset|preventDefault={onResetForm}>
        <div class="text-sm mt-3"><sup>*</sup> - {$i18n.t('Mandatory Fields')}</div>
        <input type="hidden" bind:value="{id}" placeholder="{$i18n.t('Wird ein neues Produkt angelegt, wenn leer')}">

        <label class="mb-0 font-bold" for="name">{$i18n.t('Product name')}<sup>*</sup></label>
        <input type="text" bind:value="{name}" on:keydown={setDisabled}>

        <label class="mb-0 font-bold" for="categories">{$i18n.t('Categories')}<sup>*</sup></label>
        <input type="text" bind:value="{categories}" placeholder="ℹ️ {$i18n.t('Zum Beispiel: Tibetische Rezeptur Lung')}" on:keydown={setDisabled}>

        <label class="mb-0 font-bold" for="tags">{$i18n.t('Tags')}<sup>*</sup></label>
        <input type="text" bind:value="{tags}" placeholder="ℹ️ {$i18n.t('Zum Beispiel: Shake, Shape Classic, Abnehmen, Wohlfühlen , Konzept')}" on:keydown={setDisabled}>
        ...
        <label class="mb-0 font-bold" for="short_description">{$i18n.t('Short description')}<sup>*</sup></label>
        <Textarea bind:value="{short_description}" on:keydown={setDisabled}/>
        ...
        <label class="mb-0 font-bold" for="intake_recommendation">{$i18n.t('Intake recommendation')}<sup>*</sup></label>
        <Textarea bind:value="{intake_recommendation}"  on:keydown={setDisabled}/>

        <label class="mb-0 font-bold" for="ingredients">{$i18n.t('Ingredients')}<sup>*</sup></label>
        <Textarea bind:value="{ingredients}" on:keydown={setDisabled}/>
        ...
        <label class="mb-0 font-bold" for="reference_link">{$i18n.t('Product webpage')}<sup>*</sup></label>
        <input type="text" bind:value="{reference_link}" placeholder="ℹ️ {$i18n.t('Direkter Link zum Produkt im Shop oder Webseite')}" on:keydown={setDisabled}>
