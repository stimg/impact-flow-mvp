categories = [
    "Beauty & Lifestyle",
    "Body & Clean",
    "Bundles",
    "Chinesische Rezepturen",
    "Ethno Health Coach",
    "Ethno-Events",
    "Ethno-Hausapotheke",
    "Ethno-Rezepturen",
    "Ethno-Rezepturen, Für Kinder geeignet",
    "Ethno-Testsatz",
    "Für Kinder geeignet",
    "Omega Go!",
    "Omega-Öle & Vitalkomplex ",
    "Omega-Öle & Vitalkomplex, Für Kinder geeignet",
    "Produktübersicht",
    "Shape Classic, Shape Weight Management",
    "Shape Weight Management",
    "Tibetische Rezeptur Lung",
    "Vitality & Family",
    "Vorzugspaket TCR",
]

product_names = [
    "Algenkraft",
    "Algenkraft",
    "Alka Box",
    "Astragalus 10",
    "Augenkraft",
    "Ayurveda Balance",
    "Beauty Complex",
    "BEAUTY Duo",
    "Blutdruck-Komplex",
    "Blutzucker-Komplex",
    "Bupleurum 9",
    "Burner",
    "COLLAGEN Shots Refill (100 Stück)",
    "Chili choco shake",
    "Cholesterin-Komplex",
    "Cythula 12",
    "Daily 365",
    "Darmkraft",
    "Eiweiss-Vitalkomplex",
    "Enzymkraft",
    "Ethno Health Coach",
    "Ethno-Hausapotheke",
    "Ethno-Testsatz",
    "Forsythiae 10",
    "Frauenkraft",
    "Gehirnkraft",
    "Gelenkkraft",
    "HS-Omega-3 Index-Selbsttest",
    "Herzkraft",
    "Immunkraft",
    "Inflam-Komplex",
    "Lebensfreude",
    "Leberkraft",
    "Lung - inkl. Präsentbox",
    "Lungenkraft",
    "MSM greens",
    "Männerkraft",
    "OPC-Kraft",
    "Omega 3 orange",
    "Omega 3 plus",
    "Omega Duo",
    "Omega Go!",
    "Pilzkraft",
    "Polygoni 7",
    "Rehmannia 6",
    "Schisandra 13",
    "Shape Classic",
    "Vorzugspaket TCR",
    "Weidenkraft",
    "Wurzel-Komplex",
    "Zellschutz-Komplex",
    "Ziziphus 9",
    "alka duo",
    "daily B-complex",
    "daily minerals",
    "daily zenergy",
    "essence aminos",
    "fresh vanilla shake",
    "wild berry shake",
]

product_properties = [
    "combinable_with",
    "target_audience",
    "application_area",
    "ingredients",
    "formulation_origin",
    "user_experience",
    "intake_recommendation",
    "history"
]

recommendation_tags = [
    "Allergien",
    "Alterung",
    "Angstzustände, Angst",
    "Anspannung",
    "Antibiotikatherapie",
    "Antriebslosigkeit",
    "Appetitlosigkeit",
    "Arteriosklerose",
    "Arthritis",
    "Arthrose",
    "Asthma",
    "Augenprobleme (Sehschwäche, Rötung)",
    "Autoimmunerkrankungen",
    "Bandscheibenvorfall",
    "Bauchspeicheldrüsenentzündung",
    "Blutarmut",
    "Blutgerinnung, Thrombose",
    "Bluthochdruck",
    "Bronchitis",
    "Burnout",
    "Candidose, Pilzinfektion",
    "Chemotherapie",
    "Darmentzündung, Darmbeschwerden",
    "Darmflorastörung",
    "Depressionen",
    "Diabetes",
    "Durchblutungsstörungen",
    "Erkältung, Virusinfekt, Grippe",
    "Erschöpfung, Müdigkeit",
    "Faszien-Verklebungen",
    "Fettstoffwechselstörung",
    "Fibromyalgie, Faser-Muskel-Schmerz",
    "Frieren, Frösteln, Kälte",
    "Gastritis",
    "Gehirnfunktionen",
    "Gelenkschmerzen",
    "Gereiztheit, Widerstand",
    "Gicht",
    "Grübeln, zu viel Denken",
    "Haarverlust",
    "Hauterkrankungen",
    "Hepatitis",
    "Herzinfarkt, Prophylaxe",
    "Herzinsuffizienz",
    "Herzkranzgefässerkrankung",
    "Herzrhythmusstörung",
    "Herzunruhe, Herzrasen",
    "Hörschwäche",
    "Immunstärkung",
    "Kalte Hände und Füsse",
    "Konzentrationsstörungen",
    "Kopfschmerzen, Migräne",
    "Krebserkrankungen",
    "Leberbelastung",
    "Leistungsschwäche",
    "Lungenschwäche, Luftnot, flache Atmung",
    "Magenschmerzen",
    "Makuladegeneration",
    "Muskelschwäche",
    "Muskelverspannungen",
    "Muskelzuckungen",
    "Nachtschweiss",
    "Nackenverspannungen, Nackenschmerzen",
    "Potenzprobleme",
    "Prostatavergrösserung",
    "Rheuma",
    "Rückenschmerzen",
    "Schlafstörungen",
    "Schlaganfall",
    "Schmerzen, starke Schmerzen",
    "Schwache Glieder",
    "Schwindel",
    "Sodbrennen",
    "Taubheitsgefühle",
    "Tinnitus, Ohrengeräusche",
    "Unfruchtbarkeit",
    "Unruhe, Stress",
    "Verdauungsstörungen",
    "Vergesslichkeit",
    "Vergiftungen, toxische Belastungen",
    "Völlegefühl",
    "Wechseljahrsbeschwerden, Wechseljahrsprobleme",
    "Zahnprobleme",
    "Zwänge, Phobien",
]

questions = {
    "chat": [
        # Ours
        "Welche Kategorien habt ihr in Sortiment?",
        "Zeige mir alle eure Kategorien an.",
        "Welche Produkte gibt es in der Kategorie Körper & Reinigung?",
        "Zeig mir die Produkte aus der Kategorie Vitalität & Familie.",
        "Was gibt es in der Kategorie Omega-Öle?",

        "Welche Produkte sind am besten für Sportler geeignet?",
        "Für wen ist das Produkt gedacht?",

        "Welche Produkte enthalten Omega-3?",
        "Gibt es Proteinpräparate?",
        "Bei welchen Produkten gibt's Protein?",

        "Welches Produkt hilft bei Allergien?",
        "Welche Produkte sind wirksam gegen Husten?",
        "Was haben Sie gegen Durchfall?",
        "Haben Sie Produkte für Klarheit und Konzentration?",
        "Gibt es Produkte für die Unterstützung der Darmgesundheit?",
        "Welche Produkte unterstützen beim Lernen?",
        "Kann das Produkt gegen Konzentrastionsstörungen helfen?",
        "Für was ist das Produkt gut?",

        "Welche Produkte wurden in Tibet hergestellt?",
        "Wo kommt das Produkt her?",
        "Woher kommt die Rezeptur?",
        "Wie ist das Produkt entstanden?",

        "Was ist da drin?",
        "Welche Inhaltsstoffe hat das Produkt",
        "Welche Zutaten hat das Produkt",

        "Was sagen die Leute über das produkt?",

        "Wie soll ich das Produkt einnehmen?",
        "Wie verwendet man das Produkt?",
        "Wann darf das Produkt nicht eingenommen werden?",
        "Gibt es Kontraindikationen bei der Einnahme vom Produkt?",

        # Product properties
        "Für wen ist dieses Produkt gedacht?",
        "Ist das Produkt auch für Kinder geeignet?",
        "Eignet sich dieses Produkt für Sportler?",
        "Können Schwangere dieses Produkt einnehmen?",
        "Wer sollte dieses Produkt nicht verwenden?",

        "Für was ist das Produkt gut?",
        "Hilft mir das Produkt bei Schlafstörungen?",
        "Kann ich das Produkt bei Kopfschmerzen verwenden?",
        "Unterstützt es meine Konzentration im Alltag?",
        "Ist das Produkt eher für den Körper oder den Geist gedacht?",
        "Kann es bei Verdauungsproblemen helfen?",
        "Hilft das Produkt auch gegen Stress?",
        "Kann das Produkt gegen Allergien eingesetzt werden?",

        "Was ist in diesem Produkt enthalten?",
        "Welche Inhaltsstoffe sind drin?",
        "Gibt es in diesem Produkt auch Omega-3?",
        "Welche Hauptwirkstoffe sind in diesem Produkt?",
        "Sind die Inhaltsstoffe rein pflanzlich?",

        "Woher kommt diese Rezeptur?",
        "Auf welcher Grundlage basiert das Produkt?",
        "Wer hat diese Rezeptur entwickelt?",
        "Wie ist die Rezeptur entstanden?",

        "Was sagen die Leute über dieses Produkt?",
        "Gibt es Erfahrungsberichte dazu?",
        "Welche Erfolgsgeschichten gibt es mit dem Produkt?",
        "Hat das Produkt gute Rückmeldungen von Kunden?",
        "Wie fühlen sich die Menschen nach der Einnahme?",
        "Welche Erfahrungen haben Sportler mit dem Produkt gemacht?",
        "Haben Kunden über Nebenwirkungen gesprochen?",

        # Categories
        "Welche Produkte gehören in die Kategorie Beauty & Lifestyle?",
        "Gibt es hier etwas für Haut und Haare?",
        "Welche Produkte sind für Schönheit gedacht?",
        "Was finde ich im Bereich Lifestyle?",
        "Welche Beauty-Produkte bieten Sie an?",
        "Welche Produkte gehören in Body & Clean?",

        "Welche Bundles gibt es?",
        "Gibt es Sparpakete mit mehreren Produkten?",

        "Welche Produkte gibt es mit chinesischen Rezepturen?",

        "Was ist der Ethno Health Coach?",
        "Was bietet mir der Ethno Health Coach?",

        "Welche Produkte gehören in die Ethno-Hausapotheke?",
        "Was sind typische Produkte für eine Hausapotheke?",

        "Welche Ethno-Rezepturen bieten Sie an?",

        "Was ist ein Ethno-Testsatz?",
        "Wofür verwendet man den Testsatz?",
        "Wie funktioniert ein Testsatz?",

        "Was gehört in die Kategorie Omega Go!?",

        "Welche Produkte gibt es bei Omega-Öle & Vitalkomplex?",

        "Gibt es eine komplette Liste aller Kategorien?",

        "Welche Produkte gibt es in Shape Weight Management?",
        "Gibt es auch etwas für Gewichtskontrolle?",
        "Welche Produkte helfen beim Abnehmen?",
        "Welche Produkte gehören zu Shape Weight Management?",

        "Gibt es hier Proteinprodukte?",
        "Was hilft beim Abnehmen?",
        "Welche Produkte unterstützen den Stoffwechsel?",

        "Welche Produkte gibt es zur tibetischen Rezeptur Lung?",
        "Welche Inhaltsstoffe stecken in der Rezeptur Lung?",
        "Wofür ist die Rezeptur Lung gedacht?",
        "Gibt es Erfahrungsberichte zu Lung?",

        "Welche Produkte gehören in Vitality & Family?",
        "Welche Produkte stärken Vitalität?",

        "Was ist das Vorzugspaket TCR?",
        "Welche Produkte sind in TCR enthalten?",
        "Für wen ist das TCR-Paket gedacht?",
        "Welche Vorteile hat das Vorzugspaket?",

        # Misc
        "Wie ist die Rückgabe und Bezahlung geregelt?",
        "Wie viele verschiedene Rezepturen bietet Ethno Health an?",
        "Welche Produktformen gibt es bei Ethno Health?",
        "Werden die Produkte auf Reinheit geprüft?",
        "Kann ich das produkt während der Schwangerschaft konsumieren?",
        "Darf das Produkt in der Schwangerschaft eingenommen werden?",
        "Ist die Einnahme der Ethno-Health Produkte erlaubt, wenn ich aktuell verschreibungspflichtige Medikamente benutze?",
        "Wie bewahre ich die Produkt auf?",
        "Wie kann ich Hilfe bekommen?",
        "Wie fange ich mit Mentora an?",
        "Warum sollte ich den Omega‑3‑Index messen lassen?",
        "Dürfen schwangere Ethno Health Produkte verwenden?",
        "Gibt es Nebenwirkungen bei der Einnahme von Produkt?",
        "Inwiefern profitieren Vegetarier und Veganer von der umfassenden Produktpalette, die Ethno Health anbietet?",
        "Dürfen Ethno-Health Produkte bei Einnahme von Medikamenten eingenommen werden?",
        "Was unterstützt meinen Fokus, wenn ich einen klaren Kopf bei der Arbeit brauche?",
        "Kann das Produkt mit anderen Nahrungsergänzungsmitteln kombiniert werden?",
        "Kann ich die Produkte günstiger einkaufen?",
        "Was hat Biohacking mit der Mentora App zu tun?",
        "Gibt es Vorzugspakete die ich günstiger kaufen kann?",
        "Können die Produkte bei einer Krebs Diagnose eingesetzt werden?",
        "Was ist Mentora?",
        "Sind die Produkte frei von bestimmten Zusätzen oder Allergenen?",
        "Was ist die Markenessenz von Ethno Health?",
        "Was ist die Philosophie von Ethno Health?",
        "Wie komme ich bei Anspannung wieder schneller zur Ruhe?",
        "Kann ich Nahrungsergänzungen während einer Krebserkrankung nutzen?",
        "Gibt es Wechselwirkungen mit Medikamenten?",
        "Sind die Produkte von Ethno Health für Vegetarier und Veganer geeignet?",
        "Was zeichnet die Rezepturen von Ethno Health aus?",
        "Woher stammen die Zutaten für Ethno Health Produkte?",
        "Gibt es Produkte mit Vitalpilzen?",
        "Gibt es Produkte speziell für den Zellschutz?",
        "Wie werden die Produkte hergestellt?",
        "Gibt es spezielle Produkte für den Stoffwechsel?",
        "Wie lange dauert die Lieferung typischerweise?",
        "Sind Ethno Health Produkte auch für Kinder geeignet?",
        "Gibt es Kombipakete oder Sparangebote?",
        "Gibt es bei dem Produkt etwas zu beachten?",
        "Sind Ethno Health Produkte vegan?",
        "Ist eine persönliche Beratung möglich?",
    ],

    "recommendations": [
        # Freie
        "Welches Produkt hilft bei Allergien?",
        "Welche Produkte sind wirksam gegen Husten?",
        "Haben Sie Produkte für Klarheit und Konzentration?",
        "Gibt es Produkte für die Unterstützung der Darmgesundheit?",
        "Welche Produkte unterstützen beim Lernen?",
        "Ich habe Husten.",
        "Was unterstützt meinen Fokus, wenn ich einen klaren Kopf bei der Arbeit brauche?",
        "Was kann ich nehmen, um mein Immunsystem im Alltag zu stärken?",
        "Welches Produkt passt, wenn ich mich einfach insgesamt etwas leichter und wohler fühlen will?",
        "Können die Produkte bei einer Krebs Diagnose eingesetzt werden?",
        "Was kann mir helfen, meine Leber sanft zu unterstützen, wenn ich mich mal entlasten will?",
        "Was könnte mir helfen, meine Haut und Zellen vor oxidativem Stress zu schützen?",
        "Wie komme ich bei Anspannung wieder schneller zur Ruhe?",
        "Wenn ich ein Produkt suche, das mir hilft, abends zur Ruhe zu kommen und besser zu schlafen, welches wäre passend?",
        "Welches Produkt hilft mir dabei, mich nach einer langen Arbeitswoche wieder etwas ausgeglichener zu fühlen?",
        "Was könnte ich nehmen, um meine Haut von innen zu unterstützen, wenn sie mal etwas mehr Pflege braucht?",
        "Kann ich Nahrungsergänzungen während einer Krebserkrankung nutzen?",
        "Ich bin in Wechselsjahren, was könnt ihr empfehlen?",

        # Allergien
        "Welches Produkt hilft zuverlässig bei Allergien?",
        "Ich habe oft juckende Haut und Niesen – was kann dagegen helfen?",
        "Gibt es etwas, das meine Allergiebeschwerden im Alltag lindert?",

        # Alterung
        "Gibt es Produkte, die den Alterungsprozess verlangsamen?",
        "Was kann ich nehmen, um jung und vital zu bleiben?",
        "Wie kann ich meine Zellen vor vorzeitiger Alterung schützen?",

        # Angstzustände, Angst
        "Ich habe öfter Angstzustände – gibt es etwas Beruhigendes?",
        "Was kann mir helfen, wenn ich innerlich sehr unruhig bin?",
        "Gibt es Produkte, die mich bei Panikgefühlen unterstützen?",

        # Anspannung
        "Was hilft mir, wenn ich ständig angespannt bin?",
        "Gibt es Produkte, die die Muskeln und den Kopf lockern?",
        "Ich komme nicht runter – was kann ich gegen Anspannung tun?",

        # Antibiotikatherapie
        "Ich mache eine Antibiotikatherapie – wie kann ich meine Darmflora schützen?",
        "Gibt es Produkte, die nach Antibiotika den Körper wieder ins Gleichgewicht bringen?",
        "Was unterstützt meinen Darm nach einer Antibiotikabehandlung?",

        # Antriebslosigkeit
        "Ich fühle mich oft antriebslos – was kann Energie bringen?",
        "Gibt es Produkte, die mir wieder mehr Schwung im Alltag geben?",
        "Wie bekomme ich meine Motivation wieder zurück?",

        # Appetitlosigkeit
        "Ich habe keinen Appetit – was könnte mir helfen?",
        "Gibt es Produkte, die den Hunger anregen?",
        "Was kann ich nehmen, wenn mir das Essen nicht mehr schmeckt?",

        # Arteriosklerose
        "Gibt es Produkte zur Unterstützung bei Arteriosklerose?",
        "Wie kann ich meine Arterien gesund halten?",
        "Was kann Ablagerungen in den Gefässen vorbeugen?",

        # Arthritis
        "Was lindert die Beschwerden bei Arthritis?",
        "Gibt es etwas, das die Gelenke beweglicher macht?",
        "Welche Produkte helfen gegen entzündete Gelenke?",

        # Arthrose
        "Ich habe Arthrose – was könnte meine Schmerzen lindern?",
        "Gibt es Produkte, die den Knorpel unterstützen?",
        "Was hilft, wenn die Gelenke steif werden?",

        # Asthma
        "Gibt es Unterstützung bei Atemnot durch Asthma?",
        "Was kann die Lunge bei Asthma stärken?",
        "Welche Produkte helfen mir, freier zu atmen?",

        # Augenprobleme (Sehschwäche, Rötung)
        "Ich habe oft rote, brennende Augen – was kann helfen?",
        "Gibt es Produkte, die meine Sehkraft unterstützen?",
        "Wie kann ich meine Augen von innen stärken?",

        # Autoimmunerkrankungen
        "Gibt es Produkte, die bei Autoimmunerkrankungen unterstützen?",
        "Wie kann ich mein Immunsystem bei einer Autoimmunerkrankung ausgleichen?",
        "Was kann helfen, wenn der Körper sich selbst angreift?",

        # Bandscheibenvorfall
        "Ich habe einen Bandscheibenvorfall – was könnte helfen?",
        "Gibt es Produkte, die die Regeneration der Wirbelsäule unterstützen?",
        "Was lindert die Schmerzen im Rücken nach einem Vorfall?",

        # Bauchspeicheldrüsenentzündung
        "Ich hatte eine Bauchspeicheldrüsenentzündung – was kann meinen Körper entlasten?",
        "Gibt es Produkte, die die Bauchspeicheldrüse sanft unterstützen?",
        "Wie kann ich mein Verdauungssystem nach einer Entzündung stärken?",

        # Blutarmut
        "Was hilft bei Blutarmut?",
        "Gibt es Produkte, die die Blutbildung unterstützen?",
        "Wie bekomme ich wieder mehr Energie bei Eisenmangel?",

        # Blutgerinnung, Thrombose
        "Gibt es Produkte, die die Blutgerinnung regulieren?",
        "Was hilft zur Vorbeugung gegen Thrombosen?",
        "Wie kann ich meine Gefässe gesund halten?",

        # Bluthochdruck
        "Ich habe Bluthochdruck – gibt es Produkte, die das Herz unterstützen?",
        "Was hilft, den Blutdruck natürlich zu senken?",

        # Bronchitis
        "Was hilft bei akuter Bronchitis?",
        "Ich habe starken Husten – gibt es etwas, das beruhigt?",
        "Welche Produkte unterstützen die Bronchien?",

        # Burnout
        "Ich fühle mich ausgebrannt – was kann helfen?",
        "Gibt es Produkte, die mich bei Burnout wieder aufbauen?",
        "Was bringt mehr Energie nach einer Stressphase?",

        # Candidose, Pilzinfektion
        "Gibt es Produkte gegen Pilzinfektionen?",
        "Was unterstützt bei einer Candidose?",
        "Wie bekomme ich meine Darmflora nach einem Pilz wieder in Ordnung?",

        # Chemotherapie
        "Ich mache eine Chemotherapie – welche Produkte können unterstützen?",
        "Gibt es etwas, das meine Kräfte während der Therapie stärkt?",
        "Was hilft, die Nebenwirkungen einer Chemotherapie zu lindern?",

        # Darmentzündung, Darmbeschwerden
        "Ich habe oft Darmentzündungen – was kann helfen?",
        "Welche Produkte beruhigen den Darm?",
        "Was unterstützt eine gesunde Verdauung bei Beschwerden?",

        # Darmflorastörung
        "Was bringt meine Darmflora wieder ins Gleichgewicht?",
        "Gibt es Produkte, die die guten Bakterien stärken?",
        "Wie regeneriere ich meinen Darm nach Störungen?",

        # Depressionen
        "Ich leide an Depressionen – gibt es natürliche Unterstützung?",
        "Was kann meine Stimmung aufhellen?",
        "Welche Produkte helfen, wieder Freude zu empfinden?",

        # Diabetes
        "Gibt es Produkte, die den Blutzucker stabilisieren?",
        "Was kann die Bauchspeicheldrüse bei Diabetes unterstützen?",
        "Wie kann ich meinen Stoffwechsel ausgleichen?",

        # Durchblutungsstörungen
        "Ich habe Durchblutungsstörungen – was kann helfen?",
        "Welche Produkte fördern die Durchblutung?",

        # Erkältung, Virusinfekt, Grippe
        "Ich habe eine Erkältung – was können Sie empfehlen?",
        "Was stärkt mich bei Grippe oder Virusinfekten?",
        "Welche Produkte helfen gegen Husten und Schnupfen?",

        # Erschöpfung, Müdigkeit
        "Ich bin ständig müde – was gibt wieder Energie?",
        "Gibt es Produkte gegen Erschöpfung?",
        "Wie komme ich morgens leichter in Schwung?",

        # Faszien-Verklebungen
        "Meine Faszien fühlen sich verklebt an – was könnte helfen?",
        "Gibt es Produkte, die die Beweglichkeit der Muskeln fördern?",
        "Wie kann ich meinen Körper bei Verklebungen unterstützen?",

        # Fettstoffwechselstörung
        "Was hilft bei Fettstoffwechselstörungen?",
        "Gibt es Produkte, die die Blutfette regulieren?",

        # Fibromyalgie, Faser-Muskel-Schmerz
        "Ich habe Fibromyalgie – was könnte Linderung bringen?",
        "Gibt es Produkte gegen Faser-Muskel-Schmerzen?",
        "Wie kann ich meine Muskeln sanft unterstützen?",

        # Frieren, Frösteln, Kälte
        "Ich friere ständig – was kann helfen?",
        "Gibt es Produkte gegen das ständige Frösteln?",

        # Gastritis
        "Ich habe Gastritis – was könnte meinen Magen beruhigen?",
        "Gibt es Produkte, die den Magen sanft schützen?",
        "Wie kann ich die Magenschleimhaut unterstützen?",

        # Gehirnfunktionen
        "Was unterstützt meine Gehirnfunktionen?",
        "Gibt es Produkte für mehr Klarheit und Fokus?",
        "Wie bleibe ich geistig fit?",

        # Gelenkschmerzen
        "Ich habe Gelenkschmerzen – was kann ich einnehmen?",
        "Welche Produkte lindern steife Gelenke?",
        "Was unterstützt meine Beweglichkeit?",

        # Gereiztheit, Widerstand
        "Ich bin oft gereizt – was bringt mich ins Gleichgewicht?",
        "Gibt es Produkte gegen innere Unruhe?",
        "Wie kann ich wieder gelassener werden?",

        # Gicht
        "Was hilft bei Gichtanfällen?",
        "Wie kann ich meine Gelenke bei Gicht schützen?",

        # Grübeln, zu viel Denken
        "Ich denke ständig zu viel nach – was beruhigt meinen Kopf?",
        "Gibt es Produkte, die das Grübeln stoppen?",
        "Wie finde ich leichter innere Ruhe?",

        # Haarverlust
        "Ich verliere viele Haare – was kann helfen?",
        "Gibt es Produkte, die das Haarwachstum fördern?",
        "Wie kann ich meine Haare von innen stärken?",

        # Hauterkrankungen
        "Ich habe Hautprobleme – was unterstützt die Haut von innen?",
        "Gibt es Produkte gegen unreine Haut?",
        "Wie kann ich meine Hautbarriere stärken?",

        # Hepatitis
        "Gibt es Produkte, die die Leber bei Hepatitis unterstützen?",
        "Wie kann ich meine Leber entlasten?",
        "Welche Produkte fördern die Lebergesundheit?",

        # Herzinfarkt, Prophylaxe
        "Was hilft zur Herzinfarkt-Prophylaxe?",

        # Herzinsuffizienz
        "Was entlastet das Herz bei Insuffizienz?",

        # Herzkranzgefässerkrankung
        "Was unterstützt die Durchblutung am Herzen?",
        "Wie kann ich mein Risiko bei Herzkrankheiten senken?",

        # Herzrhythmusstörung
        "Ich habe Herzrhythmusstörungen – was kann helfen?",
        "Gibt es Produkte, die den Herzschlag stabilisieren?",
        "Wie kann ich mein Herz beruhigen?",

        # Herzunruhe, Herzrasen
        "Mein Herz rast oft – gibt es etwas zur Beruhigung?",
        "Was kann helfen bei Herzunruhe?",
        "Gibt es Produkte, die das Herz ruhiger machen?",

        # Hörschwäche
        "Ich höre schlechter – gibt es Produkte für die Ohren?",
        "Wie kann ich mein Gehör unterstützen?",
        "Welche Produkte helfen bei Hörproblemen?",

        # Immunstärkung
        "Was stärkt mein Immunsystem dauerhaft?",
        "Gibt es Produkte, die mich weniger anfällig für Infekte machen?",
        "Wie kann ich meine Abwehrkräfte aufbauen?",

        # Kalte Hände und Füsse
        "Ich habe ständig kalte Hände – was kann ich tun?",
        "Welche Produkte helfen gegen kalte Füsse?",

        # Konzentrationsstörungen
        "Ich kann mich schlecht konzentrieren – was hilft?",
        "Welche Produkte fördern die Konzentration beim Lernen?",
        "Wie bekomme ich einen klareren Kopf?",

        # Kopfschmerzen, Migräne
        "Ich habe oft Migräne – was könnte helfen?",
        "Welche Produkte lindern Kopfschmerzen?",
        "Wie kann ich vorbeugen, dass die Schmerzen kommen?",

        # Krebserkrankungen
        "Gibt es Produkte, die mich bei einer Krebserkrankung unterstützen?",
        "Kann ich Nahrungsergänzungen während einer Krebsdiagnose nehmen?",
        "Was hilft, die Kräfte während einer Therapie zu erhalten?",

        # Leberbelastung
        "Meine Leber ist belastet – was kann sie entlasten?",
        "Gibt es Produkte zur Leberreinigung?",
        "Wie kann ich meine Leber regenerieren?",

        # Leistungsschwäche
        "Ich fühle mich leistungsschwach – was bringt neue Kraft?",
        "Wie bekomme ich meine Energie zurück?",

        # Lunge, Luftnot
        "Ich habe schwache Lungen – was könnte helfen?",
        "Was hilft bei Luftnot und flacher Atmung?",

        # Magenschmerzen
        "Ich habe Magenschmerzen – was lindert diese?",
        "Welche Produkte beruhigen den Magen?",
        "Wie kann ich meine Verdauung entlasten?",

        # Makuladegeneration
        "Gibt es Produkte gegen Makuladegeneration?",
        "Wie kann ich meine Augen bei Netzhautproblemen unterstützen?",
        "Welche Produkte helfen, die Sehkraft zu erhalten?",

        # Muskelschwäche
        "Ich habe Muskelschwäche – was könnte helfen?",
        "Gibt es Produkte, die die Muskelkraft stärken?",
        "Wie kann ich meine Muskulatur unterstützen?",

        # Muskelverspannungen
        "Ich habe starke Muskelverspannungen – was hilft?",
        "Welche Produkte lockern den Rücken?",
        "Wie kann ich Verspannungen vorbeugen?",

        # Muskelzuckungen
        "Ich habe Muskelzuckungen – was unterstützt die Nerven?",
        "Gibt es Produkte, die gegen unruhige Muskeln helfen?",
        "Wie kann ich meinen Körper ausgleichen?",

        # Nachtschweiss
        "Ich habe oft Nachtschweiss – was kann helfen?",
        "Welche Produkte regulieren das Schwitzen?",
        "Wie komme ich ruhiger durch die Nacht?",

        # Nackenverspannungen
        "Mein Nacken ist ständig verspannt – was könnte helfen?",
        "Gibt es Produkte, die Nackenschmerzen lindern?",
        "Wie bekomme ich mehr Beweglichkeit im Hals?",

        # Potenzprobleme
        "Ich habe Potenzprobleme – gibt es Produkte, die helfen?",
        "Welche Unterstützung gibt es bei Erektionsschwäche?",
        "Wie kann ich meine Potenz stärken?",

        # Prostatavergrösserung
        "Meine Prostata ist vergrössert – was unterstützt die Blase?",
        "Gibt es Produkte gegen nächtlichen Harndrang?",
        "Welche Produkte helfen der Prostata?",

        # Rheuma
        "Ich habe Rheuma – was lindert die Schmerzen?",
        "Gibt es Produkte für beweglichere Gelenke?",
        "Wie kann ich meine Rheumabeschwerden unterstützen?",

        # Rückenschmerzen
        "Ich habe Rückenschmerzen – was könnte helfen?",
        "Welche Produkte unterstützen die Rückenmuskulatur?",
        "Wie kann ich meinen Rücken stärken?",

        # Schlafstörungen
        "Ich schlafe schlecht – was kann helfen?",
        "Welche Produkte bringen mich abends besser zur Ruhe?",
        "Wie kann ich meine Schlafqualität verbessern?",

        # Schlaganfall
        "Wie kann ich einem Schlaganfall vorbeugen?",
        "Gibt es Produkte, die die Hirndurchblutung unterstützen?",
        "Welche Produkte helfen nach einem Schlaganfall?",

        # Schmerzen
        "Ich habe starke Schmerzen – was kann ich nehmen?",
        "Welche Produkte lindern Schmerzen sanft?",
        "Wie kann ich meine Schmerzen besser aushalten?",

        # Schwache Glieder
        "Meine Glieder fühlen sich schwach an – was könnte helfen?",
        "Welche Produkte bringen wieder Kraft in die Arme und Beine?",

        # Schwindel
        "Ich habe oft Schwindel – was kann helfen?",
        "Welche Produkte unterstützen den Gleichgewichtssinn?",
        "Wie kann ich Schwindelanfälle vorbeugen?",

        # Sodbrennen
        "Ich habe ständig Sodbrennen – was könnte helfen?",
        "Welche Produkte sind effektiv bei Sodbrennen?",

        # Taubheitsgefühle
        "Ich habe manchmal Taubheitsgefühle – was kann helfen?",
        "Welche Produkte fördern die Nervenfunktion?",
        "Wie kann ich die Durchblutung verbessern?",

        # Tinnitus
        "Ich habe Ohrgeräusche – was kann helfen?",
        "Welche Produkte unterstützen bei Tinnitus?",
        "Wie kann ich mein Gehör beruhigen?",

        # Unfruchtbarkeit
        "Ich habe Probleme mit der Fruchtbarkeit – was könnte helfen?",
        "Welche Produkte fördern die Zeugungsfähigkeit?",

        # Unruhe, Stress
        "Ich bin ständig gestresst – was bringt Ruhe?",
        "Welche Produkte helfen bei innerer Unruhe?",
        "Wie kann ich nach Stress schneller entspannen?",

        # Verdauungsstörungen
        "Ich habe Verdauungsprobleme – was könnte helfen?",
        "Welche Produkte unterstützen den Darm?",
        "Wie kann ich meine Verdauung regulieren?",

        # Vergesslichkeit
        "Ich bin oft vergesslich – was hilft fürs Gedächtnis?",
        "Welche Produkte stärken das Erinnerungsvermögen?",
        "Wie kann ich mein Gehirn trainieren?",

        # Vergiftungen
        "Gibt es Produkte gegen toxische Belastungen?",
        "Wie kann ich meinen Körper entgiften?",
        "Welche Produkte helfen bei Vergiftungen?",

        # Völlegefühl
        "Ich habe oft Völlegefühl – was könnte helfen?",
        "Welche Produkte machen den Magen leichter?",
        "Wie kann ich die Verdauung entlasten?",

        # Wechseljahrsbeschwerden
        "Ich bin in den Wechseljahren – was kann helfen?",
        "Welche Produkte lindern Hitzewallungen?",

        # Zahnprobleme
        "Ich habe Zahnprobleme – gibt es Produkte zur Unterstützung?",
        "Wie kann ich meine Zähne von innen schützen?",

        # Zwänge, Phobien
        "Ich leide unter Zwängen – gibt es etwas zur Unterstützung?",
        "Welche Produkte helfen bei Phobien?",
        "Wie kann ich meine Ängste besser in den Griff bekommen?"
    ]
}

mentora_hints = [
    "Nimm dir jeden Tag einen Moment, um etwas zu tun, das dir wirklich Freude macht – auch wenn es nur ein paar Minuten sind.",
    "Erinnere dich daran, genug Wasser zu trinken – manchmal ist das der einfachste Weg, um dich gleich ein bisschen frischer zu fühlen.",
    "Versuch mal, am Ende des Tages drei Dinge aufzuschreiben, für die du dankbar bist – das hebt die Stimmung oft nachhaltig.",
    "Beweg dich ein bisschen, auch wenn es nur ein kurzer Spaziergang ist – frische Luft und ein paar Schritte wirken oft Wunder.",
    "Leg zwischendurch mal dein Handy weg und gönn dir ein paar Minuten ohne Bildschirm – das hilft oft, den Kopf freizubekommen.",
    "Probier mal, bewusst tief durchzuatmen, wenn du dich gestresst fühlst – ein paar ruhige Atemzüge können wirklich Wunder wirken.",
    "Versuch, dir kleine Pausen in deinen Tag einzubauen, in denen du einfach mal nichts tust – das lädt deine Energie wieder auf.",
    "Hör auf deine innere Stimme und vertraue darauf, dass du selbst am besten weißt, was dir guttut.",
    "Gönn dir ab und zu etwas, das dich inspiriert – sei es ein gutes Buch, Musik oder einfach ein inspirierendes Gespräch.",
    "Erinnere dich daran, dass Selbstfürsorge kein Luxus ist, sondern ein wichtiger Teil deines Wohlbefindens – du hast es verdient, gut für dich zu sorgen."
]
